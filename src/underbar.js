...
(function() {
  ...
  _.filter = function(collection, test) {
    ...
    _.each(collection, function(item, index){
      if (test.call(collection, item, index)){
        result.push(item);
      }
    })
  ...
  _.map = function(collection, iterator) {
    ...
    if (Array.isArray(collection)){
      ...
      for (key in collection){
        var item = collection[key];
        result.push(iterator(item, key, collection));
      }
    }
  ...
  _.extend = function(obj) {
    ...
    for (var i = 1; i < arguments.length; i++){
      ...
      for (attributes in source){
        obj[attributes] = source[attributes];
      }
    }
  ...
  _.shuffle = function(array) {
    ...
    var copiedArray = list(array);

    for (var i = 0; i < copiedArray.length; i++){
      var randomIndex = (Math.random() * copiedArray.length) | 0;
  ...
  _.sortBy = function(collection, iterator) {
    ...
    function compare(a, b) {
      if(a === undefined) {
        ...
        return -1;
      } else if (iteratorFunction(a) < iteratorFunction(b)) {
        return -1;
      } else if (iteratorFunction(a) > iteratorFunction(b)) {
        return 1;
      } else {
    ...
    while(timesSorted !== 0) {
      ...
      for(var i = 0; i < collection.length - 1; i++) {
        if(compare(collection[i], collection[i+1]) === 1 && collection[i+1] !== undefined) {
          var tempElement = collection[i];
          collection[i] = collection[i+1];
          collection[i+1] = tempElement;
          timesSorted += 1;
        }
  ...
  _.intersection = function() {
    ...
    return _.filter(_.uniq(arguments[0]), function(item){
      return _.every(arr, function(same){
        return _.contains(same, item);
      })
    })
  };
